...
Note: vehicle status is "reserved" after this step (not "sold" — payment is still pending)
```
...
Alt: Catalog PATCH fails (best-effort) → log error, sale still completed, 200 returned
```
